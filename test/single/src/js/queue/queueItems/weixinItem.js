...
define(['Util',
        ...
        '../../../tpl/queue/weixin.tpl'],function(Util,Constants,tpl){
	var _index;
...
//        			$('.uiTab .uiTabBody .uiTabItemBody').eq(0).show();
        ...
        startTimer:function(type){
            ...
            var $text = $(this.$el).find('.conversation').find('.time');
         	var WaringTime=parseInt(_index.queue.alarm.WEIXIN_ALARMTIME);
            WaringTime=WaringTime<10?"0"+WaringTime:WaringTime;
            var releaseWaring=parseInt(_index.queue.alarm.WEIXIN_RELEASETIME)-1;
            var releaseWaring=releaseWaring<10?"0"+releaseWaring:releaseWaring;
         	var releaseTime=parseInt(_index.queue.alarm.WEIXIN_RELEASETIME);
            var releaseTime=releaseTime<10?"0"+releaseTime:releaseTime;
          	var agentAlarmTime = parseInt(_index.queue.alarm.WEIXIN_AGENT_ALARMTIME);
          	if ('new'==type) {
        		this.setBackGround('new');
			}else{
				clearInterval(this.timer);
		        ...
		        if ('00'==mText && agentAlarmTime==sText) {
					if(sessionStatus!='04' && this.hasWaitMsg && !this.hasTip) {
						var data={
			            		...
			            		content:_index.queue.alarm.WEIXIN_AGENT_ALARMMSG.substr(0,17),
			            		contentTxt:_index.queue.alarm.WEIXIN_AGENT_ALARMMSG.substr(0,17),
					...
					if(sessionStatus!='04' && this.changeTipFlag && !this.hasWaitMsg && !this.hasTip && this.changeTip){
						var data={
	      					serialNo:serialNo,
	      					channelID:this.options.channelID,
	            			content:_index.queue.alarm.WEIXIN_AGENT_ALARMMSG.substr(19),
	            			contentTxt:_index.queue.alarm.WEIXIN_AGENT_ALARMMSG.substr(19),
	            			msgType:Constants.MSGTYPE_TEXT,
	            			mediaID:Constants.MEDIA_ONLINE_SERVICE,
	            			type:'noReturn',
	  		            	senderFlag:Constants.SENDER_FLAG_SYSTEM,
		        ...
		        if (WaringTime==mText && '00'==sText) {
					...
					if (sessionStatus!='04' && this.hasTip) {
		          		var data={
		      				serialNo:serialNo,
		      				channelID:this.options.channelID,
		            		content:_index.queue.alarm.WEIXIN_ALARMMSG,
		            		contentTxt:_index.queue.alarm.WEIXIN_ALARMMSG,
		            		msgType:Constants.MSGTYPE_TEXT,
		            		mediaID:Constants.MEDIA_ONLINE_SERVICE,
		            		type:'noReturn',
		        ...
		        if (releaseTime==mText && '00'==sText && this.hasTip) {
		          	if (sessionStatus!='04') {
		          		var data={
		      				serialNo:serialNo,
		      				channelID:this.options.channelID,
		            		content:_index.queue.alarm.WEIXIN_RELEASEMSG,
		            		contentTxt:_index.queue.alarm.WEIXIN_RELEASEMSG,
		            		msgType:Constants.MSGTYPE_TEXT,
		            		mediaID:Constants.MEDIA_ONLINE_SERVICE,
		            		type:'noReturn',
		            		//提示语和451消息都有可能放在消息容器中，座席发送的不可能进入消息容器
