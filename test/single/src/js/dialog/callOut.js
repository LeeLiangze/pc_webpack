define(['Util','../index/constants/mediaConstants','../log/transferOutLog','../../tpl/comMenu/callOut/callOut.tpl','../../assets/css/comMenu/callOut/callOut.css'],function(Util,MediaConstants,TransferOutLog,callOutTpl){
	var objClass = function(index,options){	
		...
		var _index = index;
		var _options = options;//接收外呼调用外呼时,传递的主被叫号码
		var callOutStr = callOutTpl;
		var callOutTemplate = Util.hdb.compile(callOutStr);
		//外呼客户未接听或直接挂断所需主被叫 start
			var callerNoForClientHangUp="";
			var calledNoForClientHangUp="";
		//外呼客户未接听或直接挂断所需主被叫 end
		//初始化主叫号码下拉框
		...
		//设置主叫.传递的参数是否有主叫号码,否则设置默认值
		var callerNum = options && _options.callerDigits ? _options.callerDigits:callcallerData[0];
		this.$el.find('.call-out-show-number-common').text(callerNum);
		/*var callOutType = _index.ctiInit.AudioCallIds.getCallOutType();//外呼类型是不是互联网会话协同外呼
		if(calledData.length>=1 && callOutType == "2"){
			this.$el.find('.call-out-show-number-panpel').val(calledData[0]);
		}*/
		if(_options && _options.calledDigits){
			this.$el.find('.call-out-show-number-panpel').val(_options.calledDigits);
		}
		//下拉选择的ui控制
		this.$el.on({
			'click':function(e){
...
//				},this),200)
		...
		$('body').on({
			'click':function(e){
				e.stopPropagation();
				$('.call-out-more-number-panpel .call-out-number').parents('.call-out-more-number').hide();
			}
		})
		///enter end
		this.$el.on({
		...
		this.$el.on('click','.call-out-call',function(e){
			//外呼客户未接听或直接挂断所需主被叫 start
			callerNoForClientHangUp= $('.call-out-show-number-common').text();//$('.call-out-show-number-common').text($.trim($(this).text()));
			calledNoForClientHangUp= $('#calledNumInput').val();
			index.ctiInit.AudioCallIds.callerNoForClientHangUp = callerNoForClientHangUp;
			index.ctiInit.AudioCallIds.calledNoForClientHangUp = calledNoForClientHangUp;
			//外呼客户未接听或直接挂断所需主被叫 end
			_index.CallingInfoMap.setIsClickTidyStatus("0");//初始化整理态状态值
			//清除延时器 start by yuexuyang
			var tidyAlertTime=_index.agentStateChangeEventTidy.flagtidyAlertTime;
			var tidyErrorTime=_index.agentStateChangeEventTidy.flagtidyErrorTime;
			clearTimeout(tidyAlertTime);
			clearTimeout(tidyErrorTime);
			//清除延时器 end
...
//				        },
		                success : function(json){
		                	...
		                	if(json.result == "0"){
		                		...
		                		if(_index.CTIInfo.audioType=="2"){
		                			_index.comMenu.comUI.setAppointedBtnEnabled("endCallBtn",true);
		                			_index.ctiInit.AudioCallIds.setWaitingToAnswer(true);
		                		}
