...
function(Util,tpl) {
	...
	var addDataInfoInit = function(){
		...
		Util.ajax.postJson('front/sh/common!execute?uid=customerInfo001',{},function(json,status){
			...
			$("#addInfo_dataSourceId2",$el).empty();
			$("#addInfo_dataSourceId3",$el).empty();
			var option = [] ;
			...
			$("#addInfo_dataSourceId2").html(configValue(""));
			$("#addInfo_dataSourceId3").html(configValue(""));
		});
	...
	var submit = function(){
		...
		var data = {
				...
				"secdDataSrcId":$el.find(".addInfo_dataSourceId_2").val(),
				"secdDataSrcName":$el.find(".addInfo_sourceField_2").val(),
				"thrdDataSrcId":$el.find(".addInfo_dataSourceId_3").val(),
				"thrdDataSrcName":$el.find(".addInfo_sourceField_3").val(),
				"pId":pId
		}
