...
//		skillsInfo = param.skillsInfo;
		skillsInfos = _index.CTIInfo.skillInfos;
//		signInSkill = param.signInSkill; 
		signInSkill = _index.CTIInfo.signInSkills;
		signId = [];
		signName = [];
		if(signInSkill){
	...
	var resetSkill_true = function(){
		...
		for(var i=0;i<$(".resetSkill .imgs").length;i++){
			if($(".resetSkill .imgs").eq(i).attr("checks") == "checked"){
				skills.push({"skillId":$(".resetSkill .imgs").eq(i).attr("skillId"),"skillName":$(".resetSkill .imgs").eq(i).next().html()});
				skillName.push({"skillName":$(".resetSkill .imgs").eq(i).next().html()});
			}
		}
		var skillNew={
    			"signInSkills":skills
...
//		Util.dialog.tips("重设技能成功");
		...
		_index.comMenu.comUI.checkInUi();//签入 变签出
		_index.destroyDialog();
		//日志
				...
				tabs : [ 
					{
						...
						click : function(e, tabData) {
							...
							collectModule&&collectModule.hide();
							$("#chatBox_error").show();
							require(['js/content/businessTree/businessTree'],function(BusinessTree){
								if(treeModule){
									treeModule.show();
					...
					}, 
					{
						...
						click : function(e, tabData) {
							...
							$("#chatBox_error").hide();
							var favorite = null;
							require(['js/content/businessTree/favorite'],function(Favorite){
								if (collectModule){
									collectModule.show();
									collectModule.empty();
